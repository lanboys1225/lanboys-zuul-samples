 ...
 * @author Denys Ivano
 */
public class LogSimpleHostRoutingFilter extends SimpleHostRoutingFilter
        implements ApplicationListener<EnvironmentChangeEvent> {

    private static final Log log = LogFactory.getLog(LogSimpleHostRoutingFilter.class);
    ...
    public LogSimpleHostRoutingFilter(ProxyRequestHelper helper, ZuulProperties properties,
    ...
    private void initialize() {
        if (!customHttpClient) {
            ...
            this.connectionManagerTimer.schedule(new TimerTask() {
                @Override
                public void run() {
                    if (LogSimpleHostRoutingFilter.this.connectionManager == null) {
                        return;
                    }
                    LogSimpleHostRoutingFilter.this.connectionManager
    ...
    public Object run() {
        ...
        try {
            ...
            RequestContext.getCurrentContext().set("zuulResponse", response);
            this.helper.setResponse(response.getStatusLine().getStatusCode(),
                    contentData == null ? null : new ByteArrayInputStream(contentData),
                    revertHeaders(response.getAllHeaders()));
        } catch (Exception ex) {
    ...
    private byte[] log(HttpServletRequest request, MultiValueMap<String, String> headers,
            MultiValueMap<String, String> params, CloseableHttpResponse response) throws IOException {
        ...
        try {
            ...
            InputStream entity = response.getEntity() == null ? null : response.getEntity().getContent();
            if (entity == null) {
                log.info("responseBody : response.getEntity() is null");
                return null;
            }
